...
def bin_func(lcgVbe, sigma=2.0, corr=0.0, bin_width=1):
	...
	for i in np.arange(58000, 59150, bin_width):
...
for sig in sigmas:
	...
	#writes to a file
	with open('oct20_bin_results_proxima_{}_{}.tsv'.format(sig, bins_width), 'wt') as f:
		csv_writer = csv.writer(f, delimiter="\t")

		csv_writer.writerow(["MDJ", "mag_gF", "err_gF", "cut_gF", "mag_gE", "err_gE", "cut_gE", 
			"mag_gi", "err_gi", "cut_gi", "mag_gm", "err_gm", "cut_gm", 
			"mag_gj", "err_gj", "cut_gj", "mag_gn", "err_gn", "cut_gn"])
		...
		for i in range(len(binned_gA[0])):
			csv_writer.writerow([format(all_mjd[i], '.3f'), 
				...
				format(b_gn[i], '.3f'), format(b_gn_err[i], '.3f'), format(binned_gn[3][i], '.3f'),])
	...
	with open('oct20_scaled_results_proxima_{}_{}.tsv'.format(sig, bins_width), 'wt') as f:
		csv_writer = csv.writer(f, delimiter="\t")

		csv_writer.writerow(["scaling factors: gi: const gF: {} gE: {} gi: const gm: {} gj: {} gn: {} Chi square: {}".format(bestA, bestE, bestm, bestq, bestn, bestX)])
		...
		for i in range(len(binned_gA[0])):
			csv_writer.writerow([format(all_mjd[i], '.3f'), 
				...
				format(scaled_gn[i], '.3f'), format(binned_gn[2][i], '.3f'), format(binned_gn[3][i]+bestn, '.3f'),
				format(scaled_ave[i], '.3f'), format(scaled_ave_err[i], '.3f')])

name2 = 'oct20_tabulated_results_proxima.tsv'
with open(name2, 'wt') as f:
	...
	for i in range(len(sigmas)):
		csv_writer.writerow([sigmas[i], format(shifts_gE[i], '.3f'), format(shifts_gA[i], '.3f'), 
			format(shifts_gm[i], '.3f'), format(shifts_gq[i], '.3f'), format(shifts_gn[i], '.3f'), 
			format(chis[i], '.3f'), format(fdl_gA[i], '.3f'), 
			format(fdl_gE[i], '.3f'), format(fdl_gi[i], '.3f'), format(fdl_gm[i], '.3f'), 
			format(fdl_gq[i], '.3f'), format(fdl_gn[i], '.3f'), format(mean_fdl[i], '.3f')])
